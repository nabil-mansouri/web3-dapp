...
contract LandBaseToken is ERC721BaseToken {
    ...
    function _regroup3x3(address from, address to, uint256 x, uint256 y, bool set) internal returns (bool) {
        ...
        if(set) {
            if(!ownerOfAll) {
                require(
                    _owners[quadId] == uint256(uint160(from)) ||
                    _owners[LAYER_6x6 + (x/6) * 6 + ((y/6) * 6) * GRID_SIZE] == uint256(uint160(from)) ||
                    _owners[LAYER_12x12 + (x/12) * 12 + ((y/12) * 12) * GRID_SIZE] == uint256(uint160(from)) ||
                    _owners[LAYER_24x24 + (x/24) * 24 + ((y/24) * 24) * GRID_SIZE] == uint256(uint160(from)),
    ...
    function _regroup24x24(address from, address to, uint256 x, uint256 y, bool set) internal returns (bool) {
        ...
        return ownerOfAll || _owners[quadId] == uint256(uint160(from));
    }
        ...
        if (owner1x1 != 0) {
            ...
            if (owner3x3 != address(0)) {
                ...
                if (owner6x6 != address(0)) {
                    ...
                    address owner12x12 = address(uint160(_owners[LAYER_12x12 + (x/12) * 12 + ((y/12) * 12) * GRID_SIZE]));
    ...
    function _ownerAndOperatorEnabledOf(uint256 id) internal override view returns (address owner, bool operatorEnabled) {
        require(id & LAYER == 0, "Invalid token id");
        uint256 x = id % GRID_SIZE;
        uint256 y = id / GRID_SIZE;
        uint256 owner1x1 = _owners[id];
